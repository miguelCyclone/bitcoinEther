...
contract TestAdvancedToken {
 ...
 MyAdvancedToken bitcoinEther = MyAdvancedToken(DeployedAddresses.MyAdvancedToken());
...
function testBlockNotAddedOk() public{
   uint256 currentBlock = block.number;
   bool blockAdded = bitcoinEther.blockAdded(currentBlock);
   Assert.isFalse(blockAdded, "The blockcnumber has already been added NoK");
 }
 ...
 function testCheckRewards() public {
   ...
   uint256 rewardGivenToMiner = bitcoinEther.balanceOf(block.coinbase) - previousBalanceMiner;
   uint256 rewardGivenToCaller = bitcoinEther.balanceOf(thisAddress) - previousBalanceCaller;

   Assert.equal(minerReward, rewardGivenToMiner, "Miner got a different reward");
   Assert.equal(callerReward, rewardGivenToCaller, "Caller got a different reward");
 }
