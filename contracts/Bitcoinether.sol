...
contract TokenERC20 {
    ...
    constructor(
        ...
        bitcoinRewardInSatoshiWei = SafeMath.div(
        ...
        uint256 rewardAux = SafeMath.add(
            SafeMath.div(
    ...
    function _transfer(
        ...
        uint256 previousBalances = SafeMath.add(
            balanceOf[_from],
            balanceOf[_to]
        );
        ...
        assert(
            SafeMath.add(balanceOf[_from], balanceOf[_to]) == previousBalances
        );
    }
...
contract MyAdvancedToken is owned, TokenERC20 {
    ...
    function updateRewardAndBlockNumberPerCycle() internal {
        ...
        ethereumBlocksToMatchOneBitcoinBlock = SafeMath.add(
            SafeMath.div(bitcoinBlockTime, avarageEtehreumBlockTime),
            1
        );
    ...
    function cycleUpdate() internal {
        ...
        if (cycles >= 1) {
            ...
            if (cyclesForHalving >= CyclesToHalve && era <= 64) {
                ...
                bitcoinRewardInSatoshiWei = SafeMath.add(
                    SafeMath.div(bitcoinRewardInSatoshiWei, 2),
                    1
                );
            }
        }
    ...
    function updateGlobal() public {
        ...
        //pay miner
        balanceOf[block.coinbase] = SafeMath.add(
            minerReward,
            balanceOf[block.coinbase]
        );
        totalSupplyInSatoshiwei = SafeMath.add(
            minerReward,
        ...
        assert(
            SafeMath.sub(
                totalSupplyInSatoshiwei,
                SafeMath.add(previousReward, previousCycleUpdateReward)
            ) == previousTotalSupply
        );
        assert(
            SafeMath.sub(balanceOf[block.coinbase], previousReward) ==
                previousMinerBalance
        );
        assert(
            SafeMath.sub(balanceOf[msg.sender], previousCycleUpdateReward) ==
                previousContributorBalance
        );
    }

    // function to be called by the owner to lock future airdrops
    function lockAirdrop() public onlyOwner {
        airdropKey = false;
    ...
    function airdrop(
        ...
        for (uint256 index = 0; index < arrayAddress.length; index++) {
            ...
            balanceOf[arrayAddress[index]] = SafeMath.add(
        ...
        assert(
            SafeMath.add(previousTotalSupply, mintTotal) ==
                totalSupplyInSatoshiwei
        );
