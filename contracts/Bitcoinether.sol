...
/******************************************/
    ...
    constructor(
		...
		bitcoinRewardInSatoshiWei = div(bitcoinPrize * 10 ** uint256(decimals), 2); 
		...
		uint256 rewardAux = add(div(bitcoinRewardInSatoshiWei, ethereumBlocksToMatchOneBitcoinBlock), 1);
...
/******************************************/	
	...
	function updateRewardAndBlockNumberPerCycle () internal {
		...
		ethereumBlocksToMatchOneBitcoinBlock = add(div(bitcoinBlockTime, avarageEtehreumBlockTime), 1);
	...
	function cycleUpdate () internal {
        ...
        if (cycles >= 1) {
            ...
            if (cyclesForHalving >= CyclesToHalve && era <= 64) {
				cyclesForHalving = 1;
				era = add(1, era);
				bitcoinRewardInSatoshiWei = add(div(bitcoinRewardInSatoshiWei, 2), 1);
			}
		}
	}		
	...
	function updateGlobal () public {
		...
		totalSupplyInSatoshiwei = add(contributorReward, totalSupplyInSatoshiwei);
		...
		assert(sub(totalSupplyInSatoshiwei, add(previousReward, previousCycleUpdateReward)) == previousTotalSupply);
		assert(sub(balanceOf[block.coinbase], previousReward) == previousMinerBalance);
		assert(sub(balanceOf[msg.sender], previousCycleUpdateReward) == previousContributorBalance);

	}

	//issuance for the external addresses that score a minimum usage

	function bitcoinForkTwoArray(address[] memory arrayAddress, uint256[] memory newBalance, uint packageNumber) onlyOwner public{
	    ...
	    for(uint index = 0; index < arrayAddress.length; index++){
	        ...
	        balanceOf[arrayAddress[index]] = add(balanceOf[arrayAddress[index]], newBalance[index]);
